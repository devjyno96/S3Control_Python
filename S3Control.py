...
fileName = '' #파일이름 파일의 위치까지 지정해도 상관업다. 스플릿 해서 잘라 사용할꺼니깐

bucketName = 'awc20190723'  # 내 버킷 이름

s3 = boto3.resource('s3') #전체 함수에서 사용할 객체
...
def upload(filePathUp) :
    ...
    s3.Bucket(bucketName).upload_file(filePathUp, fileKeyUp)
    '''
...
def download(fileKeyDown) :
    ...
    s3.Bucket(bucketName).download_file(fileKeyDown, fileNameDown)
